...
import { REVIEW_ITEMS } from "@/lib/testimonials";

export default function DashboardTestimonials() {
  const textReviews = REVIEW_ITEMS.filter((review) => review.type === "text").length;
  const audioReviews = REVIEW_ITEMS.filter((review) => review.type === "audio").length;
  const countries = new Set(REVIEW_ITEMS.map((review) => review.country)).size;
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <p className="max-w-3xl text-sm leading-relaxed text-[#0A211F]/68 sm:text-base">
                  Review the client testimonials currently prepared for the homepage and future reviews
                  page.
                </p>
              </div>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article className="rounded-2xl border border-[#0A211F]/10 bg-[#f7f9f2] p-5">
              ...
              <p className="mt-3 text-4xl font-semibold tracking-tight text-[#0A211F]">
                {String(REVIEW_ITEMS.length).padStart(2, "0")}
              </p>
            </article>
