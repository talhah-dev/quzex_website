...
export default function WorkPortfolioSection({
  ...
  selectedPage = 1,
  selectedSearch = "",
}: WorkPortfolioSectionProps) {
  const router = useRouter();
  const pathname = usePathname();
  const [query, setQuery] = useState(selectedSearch);
  const activeCategory = selectedCategory || "All";
  const currentPage = selectedPage > 0 ? selectedPage : 1;
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      ...
      if (activeCategory !== "All") {
        currentSearchParams.set("category", activeCategory);
      }

      if (selectedSearch.trim()) {
        currentSearchParams.set("search", selectedSearch.trim());
      }

      const currentHref = currentSearchParams.toString()
  ...
  }, [activeCategory, pathname, query, router, selectedSearch]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ["portfolio-cards", activeCategory, currentPage, selectedSearch],
    queryFn: () =>
  ...
  function buildCategoryHref(category: string) {
    ...
    return searchParams.toString() ? `/work?${searchParams.toString()}` : "/work";
  }
        ...
        {!isLoading && !isError && portfolioCards.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {portfolioCards.map((item) => (
              <article
                key={item._id}
                className={`min-w-0 overflow-hidden rounded-[1rem] border border-[#0A211F]/10 bg-white p-2 shadow-[0_18px_45px_-35px_rgba(10,33,31,0.35)] ${
                  item.href ? "group" : ""
                }`}
              >
                {item.href ? (
